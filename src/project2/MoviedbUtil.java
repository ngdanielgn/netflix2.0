...
public class MoviedbUtil {
		...
		try {
			...
			if (browseGenre!=null){
				...
				sql = "select m.id, m.title, m.year, m.director, s.first_name,"
						...
						+ "and gim.movie_id = m.id and (g.name = ?)";
				//System.out.println(browseGenre);
				PStat = myConn.prepareStatement(sql);
	            PStat.setString(1, browseGenre);
				// execute query
			...
			if(browseTitle!=null){
				...
				//String holder ="'";
				//holder +=browseTitle;
				//System.out.println(browseTitle);
				sql = "select m.id, m.title, m.year, m.director, s.first_name,"
						+ "s.last_name, s.id AS s_Id, g.name, g.id AS gID from movies m, genres_in_movies gim, stars s,"
						+ " stars_in_movies sim, genres g where "
						+ "sim.star_id = s.id and sim.movie_id = m.id and g.id = gim.genre_id "
						+ "and gim.movie_id = m.id and m.title LIKE ?";
				//System.out.println(sql);
				PStat = myConn.prepareStatement(sql);
	            PStat.setString(1, browseTitle + "%");
				// execute query
				myRs = PStat.executeQuery();
			}
	...
	public Movie getSingleMovie(String movieid) throws Exception {
		...
		try {
			...
			String sql ="select m.id, m.title, m.year, m.director,s.id AS s_Id, s.first_name,"
					+ "s.last_name, g.name,g.id AS gId, m.banner_url, m.trailer_url from movies m, genres_in_movies gim, stars s,"
					+ " stars_in_movies sim, genres g where "
					+ "(sim.star_id = s.id and sim.movie_id = m.id and g.id = gim.genre_id "
					+ "and gim.movie_id = m.id and m.id = ?)";
			PStat = myConn.prepareStatement(sql);
...
public Star getSingleStar(String starId) throws Exception {
		...
		try {
			...
			// create sql statement	
			String sql ="select s.id, s.dob, s.first_name, s.last_name, s.photo_url, m.id AS m_Id, m.title "
					+ "from movies m, stars s, stars_in_movies sim "
					+ "where sim.star_id = s.id and sim.movie_id = m.id and s.id = ?";
			
			PStat = myConn.prepareStatement(sql);
            PStat.setString(1, starId);
			// execute query
			myRs = PStat.executeQuery();
			while(myRs.next()) {
